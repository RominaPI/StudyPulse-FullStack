...
const priorityBadge: Record<Priority, string> = {
  low: "badge-green", medium: "badge-teal", high: "badge-coral",
};

export default function Tareas() {
  const [items, setItems] = useState<Task[]>([]);
  const [form, setForm] = useState<{ title: string; description: string; priority: Priority; dueDate: string }>({
    title: "", description: "", priority: "medium", dueDate: "",
  });
  const [err, setErr] = useState<string | null>(null);

  const load = () => api<Task[]>("/tasks").then(setItems).catch((e) => setErr(e.message));
  useEffect(() => { load(); }, []);
  ...
  return (
    <>
      <div className="topbar">
        <div><h1 className="h1">Tareas</h1><p className="muted">Gestiona tus pendientes</p></div>
      </div>
      <div className="grid grid-2">
        <div className="card">
          ...
          <form onSubmit={submit}>
            ...
            <div className="field"><label className="label">Fecha límite</label>
        ...
        <div className="card">
          ...
          <div className="list">
            {items.length === 0 && <p className="muted">Sin tareas todavía.</p>}
            {items.map((t) => (
              <div className="list-item" key={t.id}>
                <div>
                  ...
                  <span className={"badge " + priorityBadge[t.priority]} style={{ marginTop: 6 }}>
                    {t.priority}
                  </span>
                </div>
