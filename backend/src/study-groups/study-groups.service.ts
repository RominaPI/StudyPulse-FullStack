...
export class StudyGroupsService {
  ...
  async create(owner_id: string, data: Partial<StudyGroup>) {
    ...
    await this.members.save(
      this.members.create({ group_id: group.id, user_id: owner_id, role: 'owner' }),
    );
    return group;
  }
  ...
  async findByUser(user_id: string) {
    const memberships = await this.members.find({
      where: { user_id },
      relations: ['group', 'group.subject', 'group.owner'],
    });
  ...
  // Keep for admin / internal use
  findAll() {
    return this.groups.find({ relations: ['subject', 'owner'] });
  }
 
  async findOne(id: string) {
    const g = await this.groups.findOne({
      where: { id },
      relations: ['members', 'members.user', 'subject', 'owner'],
    });
  ...
  async join(group_id: string, user_id: string) {
    ...
    if (group.members.find((m) => m.user_id === user_id))
      throw new BadRequestException('Already a member');
    return this.members.save(
      this.members.create({ group_id, user_id, role: 'member' }),
    );
  }
