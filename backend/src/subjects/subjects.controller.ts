...
import { IsString, MaxLength, IsUUID, IsOptional, IsObject, IsInt } from 'class-validator';
import { SubjectsService } from './subjects.service';

class CreateSubjectDto {
  @IsString() @MaxLength(30) code: string;
  @IsString() @MaxLength(200) name: string;
  @IsUUID() university_id: string;
  @IsOptional() @IsString() description?: string;
  @IsOptional() @IsString() professor?: string;
  @IsOptional() @IsInt() credits?: number;
  @IsOptional() @IsObject() schedule?: Record<string, any>;
}
@ApiTags('subjects') @ApiBearerAuth() @Controller('subjects')
export class SubjectsController {
  constructor(private svc: SubjectsService) {}
  @Get() findAll(@Query('university_id') uni: string) { return this.svc.findByUniversity(uni); }
  @Get(':id') findOne(@Param('id', ParseUUIDPipe) id: string) { return this.svc.findOne(id); }
  @Post() create(@Body() dto: CreateSubjectDto) { return this.svc.create(dto); }
